...
public class SolutionConstraints implements Comparable<SolutionConstraints> {
	HashMap<String, HashMap<String, Long>> allocationApps;
	HashMap<String, Long> state;
	HashMap<String, Long> allocationCtm;
	...
	SolutionConstraints(HashMap<String, HashMap<String, Long>> allocationApps, HashMap<String, Long> state, HashMap<String, Long> allocationCtm, double objective, Model m){
		this.state = state;
		this.allocationApps = allocationApps;
		this.allocationCtm = allocationCtm;
	...
	public boolean equals(Object obj) {
		if (obj instanceof SolutionConstraints){
			SolutionConstraints sol = (SolutionConstraints) obj;
			return sol.state.equals(state) && sol.allocationApps.equals(allocationApps) && sol.objective == objective;
		}
		return false;
	}
	...
	public void computeMovement(Model m) {
		...
		for (Application a: EcoreUtil2.getAllContentsOfType(m, Application.class)) {
			...
			if (a.getExecutedOn() == null) {
				...
				if (!allocationApps.keySet().contains(a.getName())) {
					movedMap.put(a.getName(), true);
					nbMoved+=1;
				}
				else {
					boolean isAttributed = false;
					for (String d: allocationApps.get(a.getName()).keySet()) {
						if (allocationApps.get(a.getName()).get(d) == 1) {
							isAttributed = true;
...
//		 Compute the number and which application moved (if it's necessary)
		...
		// Apply new state for every constraint
		
		for (CounterMeasure ctm: EcoreUtil2.getAllContentsOfType(m, CounterMeasure.class)) {
			boolean newInactiveVal = !(this.allocationCtm.get(ctm.getName()).intValue() == 1);
			if (ctm.isInactive() != newInactiveVal) {
				if (newInactiveVal) {
					java.lang.System.out.println("\t countermeasure " + ctm.getName() + " is now inactive");
				}
				else {
					java.lang.System.out.println("\t countermeasure " + ctm.getName() + " is now active (cost: " + (ctm.getCost() == null ? 0 : ctm.getCost().getValue())  +")");
				}
			}
			ctm.setInactive(newInactiveVal);
...
//				java.lang.System.out.println("application " + a.getName() + ": ");
			...
			// Put the new attribution
				...
				HashMap<String, Long> appAttribution = allocationApps.get(a.getName());
	...
	public String toString() {
		...
		for (String keyA: allocationApps.keySet()) {
			...
			if (state.get(keyA) == 2) {
		    	...
		    	for (String keyD: allocationApps.get(keyA).keySet()) {
		    		if (allocationApps.get(keyA).get(keyD) == 1) {
		    			if (state.get(keyA) == 0) {
	...
	@Override
	public int compareTo(SolutionConstraints o) {
