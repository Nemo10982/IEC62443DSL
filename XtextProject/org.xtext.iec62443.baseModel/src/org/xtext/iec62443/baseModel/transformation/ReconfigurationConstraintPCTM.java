...
public class ReconfigurationConstraintPCTM { 
	...
	// Kept in this version, but only takes into account maximum level attainable (all countermeasures will not necessarily be in place).
...
//		}
	...
	static class SolutionEnumerator extends CpSolverSolutionCallback{
		...
		HashMap<String,HashMap<String,BoolVar>> runningVars;
		HashMap<String,BoolVar> constraintVars;
		private LinearExpr toMax;
		  private int solutionCount;
		  private double curMax = 0.0;
		  private List<SolutionConstraints> solutions;
		  private Model m;
		  private IntVar nbMoved;

		
		  public SolutionEnumerator(HashMap<String,HashMap<String,BoolVar>> runningVars, HashMap<String,IntVar> stateVars, HashMap<String,BoolVar> constraintVars, LinearExpr toMax, Model m, IntVar NbMoved) {
		...
		@Override
		  public void onSolutionCallback() {
			...
			HashMap<String, Long> allocationCst = new HashMap<>();

//		    java.lang.System.out.printf("Solution #%d: time = %.02f s%n", solutionCount, wallTime());
			for (String keyC: constraintVars.keySet()) {
		    	allocationCst.put(keyC, value(constraintVars.get(keyC)));
			}
...
//		    java.lang.System.out.println("moved: " + value(nbMoved) + " score " + value(toMax));
		    if (curMax == value(toMax)) {
		    	SolutionConstraints toAdd = new SolutionConstraints(allocation, state, allocationCst, value(toMax), m);
			    ...
			    curMax = value(toMax);
			    solutions.clear();
			    solutions.add(new SolutionConstraints(allocation, state, allocationCst, value(toMax),m));
			    solutionCount = 1;
		    }
		  }
	...
	public static Model reconfigureWithout(Component c, Model m, boolean askForConfirmation) {
		...
		List<CounterMeasure> countermeasures = EcoreUtil2.getAllContentsOfType(m, CounterMeasure.class);
...
//		long coefficientMovement = (applications.size() + 1);
		...
		HashMap<String,BoolVar> noneVars = new HashMap<>();
		HashMap<String,IntVar> stateVars = new HashMap<>();
		HashMap<String,HashMap<String,BoolVar>> runningVars = new HashMap<>();
		HashMap<String,HashMap<String,BoolVar>> inVars = new HashMap<>();
		
		// Activate/deActivate countermeasures
		HashMap<String,BoolVar> ctmActiveVars = new HashMap<>();
		
		
		// computed ensured security levels (by constraints)
//		HashMap<String,HashMap<Integer,IntVar>> zoneSLs = new HashMap<>();
		
		// In fact, we prefer using a list of constraint for each zone, for each SL, that are enough to achieve target.
		HashMap<String,List<List<BoolVar>>> zoneConstraints = new HashMap<>();
		...
		HashMap<String,BoolVar> movedVars = new HashMap<>();
		...
		// For every countermeasure in the model, define the active/inactive variable
		
		for (CounterMeasure ctm : countermeasures) {
			String cName = ctm.getName();
			ctmActiveVars.put(cName, model.newBoolVar(cName + "_active"));
		}

		// For every zone, define the SL variables, taking into account min and max slevels
//		
//		for (Zone z : zones) {
//			String zName = z.getName();
//			HashMap<Integer, IntVar> curZoneSL = new HashMap<Integer,IntVar>();
//			zoneSLs.put(zName, curZoneSL);
//			Integer[] sLHigh = getSLFromCountermeasures(z.getCountermeasures());
//			
//			for (int i =0; i < 7; i++) {
//				curZoneSL.put(i, model.newIntVar(0, sLHigh[i], zName + "_SL_" + i));
//			}
//		}
		
		// For every zone, give the list of active countermeasure boolean decision variables corrsponding to each SL
		
		for (Zone z : zones) {
			String zName = z.getName();
			Integer[] zlvl = getSLFromClass(z.getTarget());
			List<List<BoolVar>> curHash = new ArrayList<>();
			zoneConstraints.put(zName, curHash);
			for (int i = 0; i < 7; i++) {
				curHash.add(i,new ArrayList<>());
			}
			List<CounterMeasure> ctmL = z.getCountermeasures();
			for (CounterMeasure ctm: ctmL) {
				Integer[] clvl = getSLFromClass(ctm.getCapability());
				for (int i = 0; i <7; i++) {
					if (clvl[i] >= zlvl[i]) {
						curHash.get(i).add(ctmActiveVars.get(ctm.getName()));
					}
				}
			}
		}
		...
		for (Application a : EcoreUtil2.getAllContentsOfType(m, Application.class)) {
			...
			// additional variable
			movedVars.put(aName, model.newBoolVar(aName + "_moved"));
		}
		
		for (Application a : applications) {
			String aName = a.getName();
			//vars
			runningVars.put(aName, new HashMap<String,BoolVar>());
			inVars.put(aName, new HashMap<String,BoolVar>());
			noneVars.put(aName, model.newBoolVar(aName + "_not_executed"));
			...
			for (Device d : devices) {
				String dName = d.getName();
				//vars
				runningVars.get(aName).put(dName, model.newBoolVar(aName + "_running_in_" + dName));
			...
			for (Zone z : zones) {
				String zName = z.getName();
				//vars
				inVars.get(aName).put(zName, model.newBoolVar(aName + "_in_zone_" + zName));
		...
		List<BoolVar> AllRunning = new ArrayList<>();
		
		// Get scores, and running applications variables
		for (Application a : applications) {
			for (Device d : devices) {
		...
		// Get costs for active countermeasures variables
		for (CounterMeasure ctm : countermeasures) {
			AllRunning.add(ctmActiveVars.get(ctm.getName()));
			IntegerValue cost = ctm.getCost();
			if (cost == null){
				AllScores.add((long) 0.0);
			}
			else {
				AllScores.add((long) -cost.getValue());
			}
		}
		
		long[] scoresArray = Arrays.stream(AllScores.toArray(new Long[0])).mapToLong(Long::longValue).toArray();
		LinearExpr valueToMaximize = LinearExpr.weightedSum(AllRunning.toArray(new BoolVar[0]),scoresArray);
		model.maximize(valueToMaximize);
		
		// At most one run location, and only compatible devices
		for ( Application a : applications) {

			List<BoolVar> varsList = new ArrayList<>();
			...
			//model.addEquality(LinearExpr.sum(varsList.toArray(new BoolVar[0])), 1);
			
			// Pure CP version:
			model.addExactlyOne(varsList.toArray(new BoolVar[0]));
		}
		
		
//		// manage the ensured SL for each zone
//		// It is the max of the SL of all active countermeasures
//		for (Zone z : zones) {
//			List<BoolVar> activeVars = new ArrayList<>();
//			List<Integer[]> correspondingScores = new ArrayList<>();
//			for (CounterMeasure ctm: z.getCountermeasures()) {
//				activeVars.add(ctmActiveVars.get(ctm.getName()));
//				correspondingScores.add(getSLFromClass(ctm.getCapability()));
//			}
//			
//			for (int i = 0; i < 7; i++) {
//				List<LinearArgument> sLIfActives = new ArrayList<>();
//				for (int j = 0; j < activeVars.size(); j++) {
//					BoolVar[] singleVar = {activeVars.get(j)};
//					long[] singleVal = {correspondingScores.get(j)[i].longValue()};
//					
//					sLIfActives.add(LinearExpr.weightedSum(singleVar, singleVal));
//				}
//				model.addMaxEquality(zoneSLs.get(z.getName()).get(i), sLIfActives.toArray(new LinearArgument[0]));
//			}
//			
//		}
		...
		for (Device d : devices) {
			...
			if (CapacityCoeff.get(d.getName()) != -1) {
				model.addLessOrEqual(LinearExpr.weightedSum((BoolVar[])varsList.toArray(new BoolVar[0]), sizeArray), CapacityCoeff.get(d.getName()));
			}
			...
			for (Application a: applications) {
				...
				for (Component c2: z.getComponents()) {
					if (c2.getType() == Component.DEVICE){
						Device d = (Device) c2;
						BoolVar toAdd = runningVars.get(a.getName()).get(d.getName());
						if (toAdd == null) {
							continue;
						}
						varsList.add(toAdd);
				...
				model.addEquality(LinearExpr.weightedSum((BoolVar[])varsList.toArray(new BoolVar[0]), coefArray), 0);
			}
		}
		
		
//		// If an application is in a zone, for each security level in the vector, it either has enough capability, or needs it to be guaranteed -> new constraint.
//		for (Application a: applications) {
//			Integer[] alvl = getSLFromClass(a.getCapability());
//			for (Zone z : zones) {
//				Integer[] zlvl = getSLFromClass(z.getTarget());
//				for (int i = 0; i < 7 ; i++) {
//					if (alvl[i] < zlvl[i]) {
//						// Add constraint that forces a countermeasure to be active.
//						BoolVar[] singleInVar = {inVars.get(a.getName()).get(z.getName())};
//						IntVar[] singleSLVar = {zoneSLs.get(z.getName()).get(i)};
//						long[] singleTargetVal = {(long) zlvl[i]};
//						LinearExpr[] singleExpr = {LinearExpr.weightedSum(singleInVar, singleTargetVal)};
//						model.addGreaterOrEqual(singleSLVar, singleExpr);
//					}
//				}
//			}
//		}
		...
		// If an application is in a zone, for each security level in the vector, it either has enough capability, or needs it to be guaranteed -> new constraint forcing at least one of the corresponding countermeasures to be active.
		
		for (Zone z : zones) {
			List<List<BoolVar>> listCst = zoneConstraints.get(z.getName());
			Integer[] zlvl = getSLFromClass(z.getTarget());
			for (Application a: applications) {
				Integer[] alvl = getSLFromClass(a.getCapability());
				for (int i = 0; i < 7 ; i++) {
					List<BoolVar> ListCstLV = listCst.get(i);
					if (alvl[i] < zlvl[i]) {
						// Add constraint that forces a countermeasure to be active.
						;
						
						// Sidesteping the issue: at least one, including the negation of the attribution to the zone.
						// In any case, we're outside of the scope of LP at this point.
						List<Literal> lLit = new ArrayList<>();
						for (BoolVar b : ListCstLV) {
							lLit.add(b);
						}
						lLit.add(inVars.get(a.getName()).get(z.getName()).not());
						model.addAtLeastOne(lLit);
					}
				}
			}
		}
		...
		for (Application a: applications) {
			...
			long[] coefArray = Arrays.stream((Long[])CoefficientList.toArray(new Long[0])).mapToLong(Long::longValue).toArray();
			model.addEquality(LinearExpr.weightedSum((BoolVar[])varsList.toArray(new BoolVar[0]), coefArray), stateVars.get(a.getName()));
		}
					...
					for (Zone z: zones) {
						for (Zone z2: zones) {
							if (dMatrix.get(z.getName()).get(z2.getName()) > 1) {
								BoolVar[] twoIns = {inVars.get(a.getName()).get(z.getName()), inVars.get(a2.getName()).get(z2.getName())};
		...
		List<BoolVar> movedVarsList = new ArrayList<>();
		...
		model.addEquality(LinearExpr.sum(movedVarsList.toArray(new BoolVar[0])), NumberMoved);
		

		SolutionEnumerator enumerator =  new SolutionEnumerator(runningVars, stateVars, ctmActiveVars, valueToMaximize, m, NumberMoved);
